            ...
            {Object.entries(skillsByCategory).map(([categoryKey, categoryData], index) => (
              ...
              >
                ...
                <div className="flex items-center gap-4 mb-6">
                  <div className={`p-3 ${categoryData.color} rounded-xl`}>
                    <categoryData.icon className="w-6 h-6 text-white" />
                  </div>
                  <div className="flex-1">
                    <h3 className="text-xl font-bold text-gray-900 dark:text-dark-text">{categoryData.label}</h3>
                    <p className="text-sm text-gray-600 dark:text-dark-text-secondary">
                      {categoryData.skills.length} skills • {categoryData.averageLevel}% average proficiency
                    </p>
                  </div>
                </div>
